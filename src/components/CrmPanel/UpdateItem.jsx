...
import { selectUpdatedItem } from '@/redux/crm/selectors';
...
export default function UpdateItem({ config, UpdateForm }) {
  ...
  const { updatePanel } = crmContextAction;
  ...
  const onSubmit = (fieldsValue) => {
    ...
    dispatch(crm.update({ entity, id, jsonData: fieldsValue }));
  };
  useEffect(() => {
    if (isSuccess) {
      form.resetFields();
      setSubTotal(0);
      dispatch(crm.resetAction({ actionType: 'update' }));
      updatePanel.close();
